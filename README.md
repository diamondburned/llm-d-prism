...
- **`RESULTS_STORE_BUCKET`**: Primary GCS bucket used for reading and writing benchmark submissions in the Results Store (and IAM allowlists). An entry may include a subdirectory prefix (`bucket/path/to/dir`); if omitted, defaults to `prism-results-store/`. Defaults to `llm-d-benchmarks`.
